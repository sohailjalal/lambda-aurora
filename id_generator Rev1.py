...
logger.info("SUCCESS: Successfully connected to RDS MySQL instance")

async def idGenerate(tableName, AppName, Type, user):
    '''This method generates Id for applications namely GuideWire, Surety, Arden and APS using autoincrement and saves new id in a
    separate table for each application. It takes two parameters, first is tableName and the second is start. tableName provides
    table name in the database and start parameter provides the starting number for auto-incrementing value in the database. Method checks if table
...
async def getData():
    ''' it gets all data from sequence table and display in browser '''
    try:
        with conn.cursor() as cur:
            table_name = 'sequence'
...
async def invoke_lambda(event): 
    ...
    the table otherwise it creates a table with the name of sequence in RDS and then insert a record in that table and then get 
    all data from it and display in browser and sends body response to the updateSequence method '''
